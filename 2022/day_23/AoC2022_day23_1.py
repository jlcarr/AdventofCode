"""
Solution to Advent of Code 2022 day 23 part 1

Fairly straighforward implementation of the instructions. There were several gotchas, such as the elves no longer moving as soon as they have no neighbors. Performed the simulation with loops, including some cleverness to cycle through the order of direction proposals. Also padded only when needed, and did a final shaving before reporting the final answer.
I wonder if a solution using convolution would be possible? Perhaps a first convolution to determine directions, then a second to count total locations, then a final to actually set the positions.
"""
import time
import sys
...
def solution(input_file):
	...
	return entries.shape[0]*entries.shape[1] - (entries == '#').sum()
