"""
Solution to Advent of Code 2022 day 24 part 2

Same as part 1, but just added a special state for the exiting at the beginning. Then solve there, back, and there again, using the distances to compute the round start of the next trip. Note it is always optimal to arrive as soon as possible then wait if need be.
"""
import time
import sys
...
def solution(input_file):
	...
	for r in range(rounds):
		...
		for pos in free_pos:
			...
			if pos == (0,0): # end2
				G.add_edge((r, *pos), (-1,0))
		free_pos = new_free_pos
	#print(G.nodes())

	#print(nx.shortest_path(G, (0,-1,0), (m,n-1)))
	there = nx.shortest_path_length(G, (0,-1,0), (m,n-1))
	back = nx.shortest_path_length(G, ((there-0)%rounds,m,n-1), (-1,0))
	there2 = nx.shortest_path_length(G, ((there+back-0)%rounds,-1,0), (m,n-1))
	#print(there, back, there2)
	return there + back + there2
